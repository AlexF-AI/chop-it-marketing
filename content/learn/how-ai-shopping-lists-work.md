...
The failure mode of most pantry features is drift: the record only changes when you edit it, so within a fortnight it describes a kitchen that no longer exists and the subtraction goes wrong. The fix is automatic management, which is how [Chop it's pantry](/learn/what-is-chop-it) works: shops you add go in, ingredients come off when you complete a meal, perishables drop off after two weeks, and bulk corrections are one chat instruction rather than a tapping session.
